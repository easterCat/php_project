...
(function(window, undefined) {
	...
	jQuery.fn = jQuery.prototype = {
		...
		init: function(selector, context, rootjQuery) {
			...
			// HANDLE: $(""), $(null), $(undefined), $(false)  //出错判断
			if(!selector) {
				return this;
			}

			// Handle HTML strings	当传入的是字符串
			if(typeof selector === "string") {
				if(selector.charAt(0) === "<" && selector.charAt(selector.length - 1) === ">" && selector.length >= 3) {
					...
					match = rquickExpr.exec(selector); //exec() 方法就是用来检索字符串中正则表达式的匹配
				}
				...
				if(match && (match[1] || !context)) {
					...
					if(match[1]) {
						...
						jQuery.merge(this, aaa); //将两个数组合并一个
				...
				// HANDLE: $(DOMElement)   //对dom元素进行处理
			} else if(selector.nodeType) {
				this.context = this[0] = selector;
				this.length = 1;
				return this;

				// HANDLE: $(function)   //对函数参数的处理
			...
			return jQuery.makeArray(selector, this); //转换成数组，传入第二个参数转为自己想要的格式，例如JSON
		},
		...
		toArray: function() { //实例方法，只能给jq对象用
		...
		get: function(num) {
			return num == null ?
				...
				// Return just the object    两种方法查找正值和负值
		...
		pushStack: function(elems) {
			...
			ret.prevObject = this; //通过挂载到属性上，在通过end()方法找到栈的下一级
